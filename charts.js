...
async function wbFetch(countryCodes, indicator, opts={}){
  ...
  // data[1] is array of observations
  return data[1] || [];
}

function pickLatestByCountry(observations){
  // returns { CODE: {year, value, name} }
  const out = {};
...
function drawBars(canvas, series, opts={}){
  ...
  series.forEach((s,i)=>{
    const x = pad + (chartW/series.length)*i + (chartW/series.length - barW)/2;
