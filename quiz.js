...
/* -------------------- PWA install (Android/desktop) + iOS help -------------------- */
...
function updateInstallUI(){
  ...
  if(isIos()){
    ...
    btn.textContent='Installa su iPhone';
    help.style.display='block';
    help.textContent='Su iPhone: Condividi (quadrato con freccia) → Aggiungi a Home.';
    return;
  }
  // Android/desktop: show only when prompt available
  if(deferredInstallPrompt){
    btn.style.display='inline-flex';
    btn.textContent='Installa app (PWA)';
    help.style.display='none';
  }else{
    btn.style.display='none';
    help.style.display='none';
...
async function doInstallPwa(){
  ...
  if(!deferredInstallPrompt){
    toast('Installazione non disponibile ora (apri dal browser principale).');
    return;
  }
...
async function resetSiteCacheAndReload(){
  try{
    ...
    // Manteniamo il nome se presente
    const keepName = (localStorage.getItem(LS.name)||'').trim();
    localStorage.clear();
    if(keepName) localStorage.setItem(LS.name, keepName);
    location.reload();
  }catch(e){
    alert('Reset cache fallito: '+(e?.message||e));
  }
}
...
const SKILL_LABELS = {
  "G1_C1":"Indicatori e tassi",
  "G1_C2":"Saldi demografici",
  "G1_C3":"Densita e distribuzione",
  "G1_C4":"Fattori della distribuzione",
  "G1_C5":"Piramide delle eta",

  "G2_C1":"Tipi di migrazioni",
  "G2_C2":"Push e pull factors",
  "G2_C3":"Regolare e irregolare",
  "G2_C4":"Migrazioni in Italia",
  "G2_C5":"Migrazioni contemporanee",

  "G3_C1":"Urbanesimo e tasso di urbanizzazione",
  "G3_C2":"Definizione e funzioni della citta",
  "G3_C3":"Gerarchia urbana e citta globali",
  "G3_C4":"Conurbazione e megalopoli",
  "G3_C5":"Modelli di citta (USA, Europa, PVS)"
};
...
/* -------------------- Template standard contenuti (JSON) --------------------
   Obiettivo: avere UN formato stabile che puoi dare a qualunque AI.

   Schema: "quiz-content-pack" (schemaVersion: 1)
   - sources: elenco documenti del prof + fonti autorevoli (ONU, UNHCR, ...)
   - units: capitoli/blocchi con skills, slideLinks, questions
   - curiosities: (opzionale) curiosita'/confronti da mostrare in Allenamento

   Il loader:
   - valida il pacchetto
   - lo converte nel formato legacy interno {catalog, units}
   - auto-aggancia slideRef se mancante (keyword matching)
-------------------------------------------------------------------------- */

const STANDARD_CONTENT_SCHEMA = "quiz-content-pack";
const STANDARD_CONTENT_SCHEMA_VERSION = 1;

function isStandardContentPack(obj){
  return !!(obj && typeof obj === 'object' && obj.schema === STANDARD_CONTENT_SCHEMA && Number(obj.schemaVersion||0) >= 1);
}

function _isNonEmptyStr(s){ return typeof s === 'string' && !!s.trim(); }

function validateStandardContentPack(pack){
  if(!_isNonEmptyStr(pack.schema)) return {ok:false, msg:'Manca schema.'};
  if(Number(pack.schemaVersion||0) < 1) return {ok:false, msg:'schemaVersion deve essere >= 1.'};
  if(!Array.isArray(pack.units) || pack.units.length===0) return {ok:false, msg:'Manca units (almeno 1 capitolo).'};

  // sources (opzionale ma consigliato)
  if(pack.sources != null){
    if(!Array.isArray(pack.sources)) return {ok:false, msg:'sources deve essere un array.'};
    for(const s of pack.sources){
      if(!s || typeof s !== 'object') return {ok:false, msg:'Ogni source deve essere un oggetto.'};
      if(!_isNonEmptyStr(s.id)) return {ok:false, msg:'Ogni source deve avere id.'};
      if(!_isNonEmptyStr(s.title)) return {ok:false, msg:'Ogni source deve avere title.'};
    }
  }

  for(const u of pack.units){
    if(!u || typeof u !== 'object') return {ok:false, msg:'Unità non valida.'};
    if(!_isNonEmptyStr(u.id)) return {ok:false, msg:'Ogni unità deve avere id (es. "G4").'};
    if(!_isNonEmptyStr(u.title)) return {ok:false, msg:'Unità '+u.id+': manca title.'};

    if(u.skills != null){
      if(!Array.isArray(u.skills)) return {ok:false, msg:'Unità '+u.id+': skills deve essere un array.'};
      for(const sk of u.skills){
        if(!sk || typeof sk!=='object') return {ok:false, msg:'Unità '+u.id+': skill non valida.'};
        if(!_isNonEmptyStr(sk.id)) return {ok:false, msg:'Unità '+u.id+': ogni skill deve avere id.'};
        if(!_isNonEmptyStr(sk.name)) return {ok:false, msg:'Unità '+u.id+': skill '+(sk.id||'?')+' manca name.'};
        if(sk.keywords!=null && !Array.isArray(sk.keywords)) return {ok:false, msg:'Unità '+u.id+': skill '+sk.id+' keywords deve essere array.'};
      }
    }

    if(u.slideLinks!=null){
      if(!Array.isArray(u.slideLinks)) return {ok:false, msg:'Unità '+u.id+': slideLinks deve essere un array.'};
      for(const l of u.slideLinks){
        if(!l || typeof l!=='object') return {ok:false, msg:'Unità '+u.id+': slideLink non valido.'};
        if(!_isNonEmptyStr(l.id)) return {ok:false, msg:'Unità '+u.id+': slideLink deve avere id.'};
        if(!_isNonEmptyStr(l.title)) return {ok:false, msg:'Unità '+u.id+': slideLink '+(l.id||'?')+' manca title.'};
        if(!_isNonEmptyStr(l.sourceId)) return {ok:false, msg:'Unità '+u.id+': slideLink '+l.id+' manca sourceId.'};
        if(l.keywords!=null && !Array.isArray(l.keywords)) return {ok:false, msg:'Unità '+u.id+': slideLink '+l.id+' keywords deve essere array.'};
      }
    }

    if(!Array.isArray(u.questions) || u.questions.length===0) return {ok:false, msg:'Unità '+u.id+': mancano questions.'};
    // validazione light delle domande: il resto lo controlla validateUnitJson dopo conversione
    for(const q of u.questions){
      if(!q || typeof q!=='object') return {ok:false, msg:'Unità '+u.id+': domanda non valida.'};
      if(!_isNonEmptyStr(q.type)) return {ok:false, msg:'Unità '+u.id+': domanda manca type.'};
      if(!_isNonEmptyStr(q.prompt)) return {ok:false, msg:'Unità '+u.id+': domanda manca prompt.'};
    }
  }

  // curiosita'/confronti (opzionali)
  if(pack.curiosities!=null){
    if(!Array.isArray(pack.curiosities)) return {ok:false, msg:'curiosities deve essere un array.'};
    for(const c of pack.curiosities){
      if(!c || typeof c!=='object') return {ok:false, msg:'Curiosità non valida.'};
      if(!_isNonEmptyStr(c.id)) return {ok:false, msg:'Ogni curiosità deve avere id.'};
      if(!_isNonEmptyStr(c.text)) return {ok:false, msg:'Curiosità '+c.id+': manca text.'};
    }
  }

  return {ok:true, msg:'OK (template standard)'};
}

function _normKeywords(arr){
  if(!Array.isArray(arr)) return [];
  return arr.map(x=>String(x||'').trim()).filter(Boolean);
}

function _buildKeywordIndex(unit){
  const idx=[];
  (unit.slideLinks||[]).forEach(l=>{
    const kws=_normKeywords(l.keywords);
    if(kws.length){
      idx.push({ kind:'slideLink', link:l, keywords:kws });
    }
  });
  (unit.skills||[]).forEach(sk=>{
    const kws=_normKeywords(sk.keywords);
    if(kws.length && sk.slideRef){
      idx.push({ kind:'skill', skill:sk, keywords:kws });
    }
  });
  return idx;
}

function _inferSlideForQuestion(q, unit, keywordIndex){
  if(q.slideRef) return q; // già presente

  // 1) se skill ha slideRef predefinito
  const sk = (unit.skills||[]).find(s=>s && s.id===q.skill);
  if(sk && sk.slideRef){
    q.slideRef = sk.slideRef;
    if(!q.slideTitle) q.slideTitle = sk.slideTitle || sk.name;
    return q;
  }

  // 2) keyword matching su prompt + spiegazioni
  const hay = [q.prompt,q.rule,q.example,q.explainOk,q.explainNo].filter(Boolean).join(' ').toLowerCase();
  for(const entry of (keywordIndex||[])){
    for(const kw of entry.keywords){
      const k = String(kw||'').toLowerCase();
      if(k && hay.indexOf(k)!==-1){
        if(entry.kind==='slideLink'){
          q.slideRef = entry.link.id;
          q.slideTitle = entry.link.title;
          return q;
        }
        if(entry.kind==='skill'){
          q.slideRef = entry.skill.slideRef;
          q.slideTitle = entry.skill.slideTitle || entry.skill.name;
          return q;
        }
      }
    }
...
function standardPackToLegacyPayload(pack){
  const units=[];
  const catalog={ units: [] };
  ...
  for(const u of pack.units||[]){
    ...
    const unitObj={
      id: unitId,
      title: String(u.title||unitId).trim(),
      // skills nel formato legacy {id,name}
      skills: Array.isArray(u.skills) ? u.skills.map(sk=>({
        id: String(sk.id).trim(),
        name: String(sk.name||sk.id).trim(),
        // extra opzionali che usiamo per inference/slide link
        keywords: _normKeywords(sk.keywords),
        slideRef: sk.slideRef || null,
        slideTitle: sk.slideTitle || null
      })) : undefined,
      questions: []
    };

    const keywordIndex=_buildKeywordIndex(u);

    let n=0;
    for(const q0 of (u.questions||[])){
      n += 1;
      const q={...q0};
      // blocco e skill
      q.block = unitId;
      if(!q.skill){
        const firstSkill = (unitObj.skills && unitObj.skills[0] && unitObj.skills[0].id) ? unitObj.skills[0].id : (unitId+"_C1");
        q.skill = firstSkill;
      }
      // id
      if(!q.id) q.id = unitId + "_" + String(n).padStart(3,'0');

      // auto slide link
      _inferSlideForQuestion(q, u, keywordIndex);

      // slideRef/slideTitle possono essere stringhe: ok
      unitObj.questions.push(q);
    }

    // validazione finale col validator legacy (piu' severo)
    const v=validateUnitJson(unitObj);
    if(!v.ok){
      throw new Error('Unità '+unitId+': '+v.msg);
    }

    units.push(unitObj);
    catalog.units.push({id: unitObj.id, title: unitObj.title});
  }

  return {
    catalog,
    units,
    __standardMeta: pack.meta || null,
    __standardSources: pack.sources || [],
    __standardCuriosities: pack.curiosities || [],
    __standardPyramidPresets: pack.pyramidPresets || [],
    __standardChains: pack.chains || []
  };
}

function makeStandardContentTemplate(){
  return {
    schema: STANDARD_CONTENT_SCHEMA,
    ...
    meta: {
      ...
      notes: "Compila sources + units. Poi incolla questo JSON in Impostazioni > Nuovi capitoli (prof)"
    },
    sources: [
      {
        id: "SRC_PROF_SLIDES_1",
        title: "Titolo documento del prof (slide/appunti)",
        type: "slides",
        author: "Prof",
        year: 2026,
        url: "",
        notes: "Se non c'è un link, lascia url vuoto. L'import serve comunque per citare la fonte."
      },
      {
        id: "SRC_UNHCR",
        title: "UNHCR (dati rifugiati)",
        type: "web",
        author: "UNHCR",
        year: 2026,
        url: "https://www.unhcr.org/",
        notes: "Esempio di fonte autorevole per curiosita'/confronti."
      }
    ],
    units: [
      {
        id: "G4",
        title: "Esempio: Clima e biomi",
        prereq: ["G1"],
        skills: [
          {
            id: "G4_C1",
            name: "Definizioni base (clima vs tempo)",
            keywords: ["clima", "tempo atmosferico"],
            slideRef: "SL_G4_CLIMA",
            slideTitle: "Clima vs tempo"
          }
        ],
        slideLinks: [
          {
            id: "SL_G4_CLIMA",
            sourceId: "SRC_PROF_SLIDES_1",
            title: "Clima vs tempo",
            keywords: ["clima", "tempo atmosferico", "meteo"]
          }
        ],
        questions: [
          {
            type: "mcq",
            skill: "G4_C1",
            prompt: "Il clima descrive...",
            choices: ["Condizioni medie a lungo periodo", "Il meteo di oggi"],
            answerIndex: 0,
            rule: "Clima = condizioni medie su periodi lunghi; tempo = stato momentaneo.",
            example: "Esempio: il clima mediterraneo ha estati secche.",
            explainOk: "Giusto: clima = media su lungo periodo.",
            explainNo: "Attenzione: quello è il tempo (meteo), non il clima.",
            slideRef: "SL_G4_CLIMA"
          }
        ]
      }
    ...
    curiosities: [
      {
        ...
        blocks: ["G4"],
        keywords: ["clima"],
        text: "Lo sapevi che ... (collega il concetto delle slide a un dato reale)?",
        source: { ref: "SRC_UNHCR", url: "https://www.unhcr.org/" }
      }
    ]
  };
}
...
function downloadStandardTemplate(){
  const json = JSON.stringify(makeStandardContentTemplate(), null, 2);
  downloadTextFile('template_quiz_content_pack.json', json);
}

async function copyStandardTemplate(){
  const json = JSON.stringify(makeStandardContentTemplate(), null, 2);
  await copyToClipboard(json);
  toast('Template copiato negli appunti.');
}
function validateContentObject(obj){
  if(!obj || typeof obj !== 'object') return {ok:false, msg:'JSON non valido.'};

  // --- Nuovo formato standard (consigliato) ---
  // {
  ...
  // }
  if(isStandardContentPack(obj)){
    return validateStandardContentPack(obj);
  }
  ...
  return {ok:false, msg:'Formato non riconosciuto. Usa il template standard oppure un\'unità singola/pacchetto legacy.'};
}
...
function exportContent(){
  ...
  const m = String(date.getMonth()+1).padStart(2,'0');
  const d = String(date.getDate()).padStart(2,'0');
  downloadTextFile(`geografia_contenuti_${y}-${m}-${d}.json`, json);
  copyToClipboard(json).catch(()=>{});
...
const LS = {
  name: "geo_name",
  profile: "geo_profile_v1",
  // c’è una sola classe: teniamo comunque la chiave per chiarezza
  classId: "geo_class",

  // --- Contenuti importati (capitoli/domande) ---
  contentCatalog: "geo_content_catalog_v1",
  contentUnitPrefix: "geo_content_unit_",

  // --- Template standard (quiz-content-pack) extra ---
  contentSources: "geo_content_sources_v1",
  contentSlideLinks: "geo_content_slidelinks_v1",
  contentCuriosities: "geo_content_curiosities_v1",
  contentMeta: "geo_content_meta_v1",
  contentPyramidPresets: "geo_content_pyramid_presets_v1",
  contentChains: "geo_content_chains_v1"
  ,
  // compat (vecchi nomi usati in alcune funzioni)
  customSources: "geo_content_sources_v1",
  customCuriosities: "geo_content_curiosities_v1"
};
...
function loadCustomCatalog(){
  ...
  try{
    const obj=JSON.parse(raw);
    if(obj && Array.isArray(obj.units)) return {
      units: obj.units,
      // extra opzionali (template standard)
      sources: Array.isArray(obj.sources) ? obj.sources : undefined,
      slideLinks: (obj.slideLinks && typeof obj.slideLinks==='object') ? obj.slideLinks : undefined,
      curiosities: Array.isArray(obj.curiosities) ? obj.curiosities : undefined,
...
function saveCustomCatalog(cat){
  const obj={
    ...
    // extra opzionali (template standard)
    sources: Array.isArray(cat.sources) ? cat.sources : undefined,
    slideLinks: (cat.slideLinks && typeof cat.slideLinks==='object') ? cat.slideLinks : undefined,
    curiosities: Array.isArray(cat.curiosities) ? cat.curiosities : undefined,
    meta: (cat.meta && typeof cat.meta==='object') ? cat.meta : undefined,
  };
  _lsSet(LS.contentCatalog, JSON.stringify(obj));
}

function loadStandardSources(){
  const raw=_lsGet(LS.contentSources);
  if(!raw) return [];
  try{ const obj=JSON.parse(raw); return Array.isArray(obj) ? obj : []; }catch{ return []; }
}
function saveStandardSources(arr){
  _lsSet(LS.contentSources, JSON.stringify(Array.isArray(arr)?arr:[]));
}

function loadStandardSlideLinks(){
  const raw=_lsGet(LS.contentSlideLinks);
  if(!raw) return {};
  try{ const obj=JSON.parse(raw); return (obj && typeof obj==='object') ? obj : {}; }catch{ return {}; }
}
function saveStandardSlideLinks(map){
  _lsSet(LS.contentSlideLinks, JSON.stringify((map && typeof map==='object')?map:{}));
}

function loadStandardCuriosities(){
  const raw=_lsGet(LS.contentCuriosities);
  if(!raw) return [];
  try{ const obj=JSON.parse(raw); return Array.isArray(obj) ? obj : []; }catch{ return []; }
}
function saveStandardCuriosities(arr){
  _lsSet(LS.contentCuriosities, JSON.stringify(Array.isArray(arr)?arr:[]));
}

function loadStandardMeta(){
  const raw=_lsGet(LS.contentMeta);
  if(!raw) return null;
  try{ const obj=JSON.parse(raw); return (obj && typeof obj==='object')?obj:null; }catch{ return null; }
}
function saveStandardMeta(obj){
  _lsSet(LS.contentMeta, JSON.stringify((obj && typeof obj==='object')?obj:null));
}

function loadStandardPyramidPresets(){
  const raw=_lsGet(LS.contentPyramidPresets);
  if(!raw) return [];
  try{ const obj=JSON.parse(raw); return Array.isArray(obj) ? obj : []; }catch{ return []; }
}
function saveStandardPyramidPresets(arr){
  _lsSet(LS.contentPyramidPresets, JSON.stringify(Array.isArray(arr)?arr:[]));
}

function loadStandardChains(){
  const raw=_lsGet(LS.contentChains);
  if(!raw) return [];
  try{ const obj=JSON.parse(raw); return Array.isArray(obj) ? obj : []; }catch{ return []; }
}
function saveStandardChains(arr){
  _lsSet(LS.contentChains, JSON.stringify(Array.isArray(arr)?arr:[]));
}
...
// --- Sorgenti e curiosita' aggiunte via template standard ---
function loadCustomSources(){
  const raw=_lsGet(LS.customSources);
  if(!raw) return [];
  try{
    const obj=JSON.parse(raw);
    return Array.isArray(obj) ? obj : (obj && Array.isArray(obj.sources) ? obj.sources : []);
  }catch{
    return [];
  }
}
...
function saveCustomSources(sources){
  ...
  _lsSet(LS.customSources, JSON.stringify(arr));
}

function loadCustomCuriosities(){
  const raw=_lsGet(LS.customCuriosities);
  if(!raw) return [];
  try{
    const obj=JSON.parse(raw);
    return Array.isArray(obj) ? obj : (obj && Array.isArray(obj.items) ? obj.items : []);
  }catch{
    return [];
  }
}

function upsertCustomCuriosities(items){
  const current=loadCustomCuriosities();
  const byId=new Map();
  ...
  _lsSet(LS.customCuriosities, JSON.stringify(merged));
}
...
function applyContentPayload(obj){
  ...
  // Converte il pacchetto standard nel formato legacy interno (catalog + units)
  // e importa anche curiosita'/confronti + sorgenti (opzionali).
  if(isStandardContentPack(obj)){
    try{
      const legacy = standardPackToLegacyPayload(obj);
      // sostituiamo obj con l'equivalente legacy per il resto della pipeline
      obj = { catalog: legacy.catalog, units: legacy.units };

      // salva meta (opzionale)
      if(legacy.__standardMeta && typeof legacy.__standardMeta==='object'){
        saveStandardMeta(legacy.__standardMeta);
      }

      // salva slideLinks (id -> {sourceId,title,keywords})
      // Nota: nel legacy non conserviamo la lista slideLinks, quindi la ricostruiamo da q.slideRef.
      try{
        const sourcesTmp = Array.isArray(legacy.__standardSources) ? legacy.__standardSources : loadCustomSources();
        const defaultSlidesSourceId = (sourcesTmp||[]).find(s=>s && s.type==='slides' && s.id)?.id || (sourcesTmp||[])[0]?.id || 'SRC_PROF_SLIDES';

        const map = loadStandardSlideLinks();
        for(const u of (Array.isArray(obj.units)?obj.units:[])){
          for(const q of (u.questions||[])){
            if(q && q.slideRef){
              const id = String(q.slideRef);
              if(!map[id]){
                map[id] = {
                  id,
                  sourceId: defaultSlidesSourceId,
                  title: String(q.slideTitle || q.slideRef),
                  keywords: undefined
                };
              }
            }
        ...
        saveStandardSlideLinks(map);
      }catch(e){}
      // salva sorgenti/curiosita' extra (opzionali)
      if(Array.isArray(legacy.__standardSources) && legacy.__standardSources.length){
        saveCustomSources(legacy.__standardSources);
      }
      if(Array.isArray(legacy.__standardCuriosities) && legacy.__standardCuriosities.length){
        upsertCustomCuriosities(legacy.__standardCuriosities);
        // forza reload pool curiosita'
        try{ CURIOSITIES = null; COMPARES = null; }catch(e){}
      }

      // salva preset piramide (opzionali)
      if(Array.isArray(legacy.__standardPyramidPresets) && legacy.__standardPyramidPresets.length){
        saveStandardPyramidPresets(legacy.__standardPyramidPresets);
        try{ PYR_PRESETS = null; }catch(e){}
      }

      // salva catene (opzionali)
      if(Array.isArray(legacy.__standardChains) && legacy.__standardChains.length){
        saveStandardChains(legacy.__standardChains);
      }
    }catch(e){
...
function exportContentBundle(){
  // Esporta sempre nel template standard (compatibile con qualunque AI)
  const cat=loadCustomCatalog();
  const legacyUnits=[];
  for(const it of (cat.units||[])){
    if(!it || !it.id) continue;
    const u=loadCustomUnit(it.id);
    if(u) legacyUnits.push(u);
  }

  const sources = loadCustomSources();
  const curiosities = loadCustomCuriosities();
  const meta = loadStandardMeta() || {
    subject: 'Geografia',
    teacher: '',
    schoolYear: '2025-2026',
    createdAt: new Date().toISOString(),
    notes: 'Pacchetto esportato dall\'app. Puoi modificarlo e re-importarlo.'
  };

  // scegli una fonte "slides" di default se presente
  const defaultSlidesSourceId = (sources||[]).find(s=>s && s.type==='slides' && s.id)?.id || (sources||[])[0]?.id || 'SRC_PROF_SLIDES';

  // Converti unità legacy -> standard (senza perdere campi extra)
  const units = legacyUnits.map(u=>{
    const skills = Array.isArray(u.skills) ? u.skills.map(sk=>({
      id: String(sk.id),
      name: String(sk.name),
      keywords: Array.isArray(sk.keywords) ? sk.keywords : undefined,
      slideRef: sk.slideRef || undefined,
      slideTitle: sk.slideTitle || undefined,
    })) : undefined;

    // slideLinks: ricostruzione minima da slideRef presenti nelle domande
    const linkMap = new Map();
    for(const q of (u.questions||[])){
      if(q && q.slideRef){
        const id = String(q.slideRef);
        const title = String(q.slideTitle || q.slideRef);
        if(!linkMap.has(id)){
          linkMap.set(id, {
            id,
            sourceId: defaultSlidesSourceId,
    ...
    return {
      ...
      prereq: Array.isArray(u.prereq) ? u.prereq : undefined,
      skills,
      slideLinks: [...linkMap.values()],
      questions: Array.isArray(u.questions) ? u.questions.map(q=>({ ...q })) : []
    };
  });

  const pyramidPresets = loadStandardPyramidPresets();
  const chains = loadStandardChains();
...
function resetCustomContent(){
  ...
  _lsDel(LS.customCuriosities);
  _lsDel(LS.customSources);
  _lsDel(LS.contentMeta);
  _lsDel(LS.contentSlideLinks);
  _lsDel(LS.contentPyramidPresets);
  _lsDel(LS.contentChains);
}
...
const DEFAULT_BLOCKS = [
  { id:"G1", name:"G1 Demografia", short:"G1", prereq:[], skills:["G1_C1","G1_C2","G1_C3","G1_C4","G1_C5"] },
  { id:"G2", name:"G2 Migrazioni", short:"G2", prereq:["G1"], skills:["G2_C1","G2_C2","G2_C3","G2_C4","G2_C5"] },
  { id:"G3", name:"G3 Le citta", short:"G3", prereq:["G1"], skills:["G3_C1","G3_C2","G3_C3","G3_C4","G3_C5"] }
];
...
// SRS per skill (stile SM-2 semplificato) + compatibilita' con i vecchi salvataggi.
// Perche' e' meglio:
// - tiene un "ease" (quanto e' facile per te)
// - usa interval dinamico (1g, 6g, poi moltiplica)
// - se sbagli, riduce l'ease e riporta la skill in ripasso ravvicinato
// Nota: se troviamo record vecchi {level,due,last} li trattiamo come "legacy".
function _now(){ return Date.now(); }

function updateSRS(skillId, ok){
  if(!skillId) return;
  if(!profile.srs) profile.srs = {};
  const cur0 = profile.srs[skillId] || {};

  // Legacy -> converte a record moderno al volo
  // legacy: {level,due,last}
  let ease = (typeof cur0.ease === 'number' && isFinite(cur0.ease)) ? cur0.ease : 2.5;
  let reps = (typeof cur0.reps === 'number' && isFinite(cur0.reps)) ? cur0.reps : 0;
  let interval = (typeof cur0.interval === 'number' && isFinite(cur0.interval)) ? cur0.interval : 0;

  if((cur0.ease===undefined) && (typeof cur0.level === 'number')){
    // stima grossolana da livello
    const lvl = Math.max(0, Math.min(5, Number(cur0.level)||0));
    reps = Math.max(reps, lvl);
    interval = [1,3,7,14,30,45][lvl] || 1;
  }
  ...
  const due = _now() + interval*24*60*60*1000;
  profile.srs[skillId] = {
    ease: ease,
    reps: reps,
    interval: interval,
...
/* -------------------- Catene di domande (definizione -> applicazione -> realta' -> orale) -------------------- */
function skillOf(q){
  if(!q) return null;
  if(q.type==='guided'){
    if(q.skills && q.skills.length) return q.skills[0];
...
function makeOralQuestion(blockId, skillId, title){
  return {
    id: `oral-${blockId}-${skillId}-${Math.floor(Math.random()*1e9)}`,
    block: blockId,
    type: 'oral',
    skill: skillId,
    prompt: `INTERROGAZIONE (orale): ${title || 'Spiega a voce'}\n\n1) Definizione\n2) 1 esempio\n3) Collegamento alla realtà (dato + fonte)\n4) Perché è importante`,
    rule: 'Usa un linguaggio geografico preciso (termini del prof).',
    example: 'Puoi aiutarti con le parole chiave: definizione → cause → effetti → esempio reale.',
    explainOk: 'Bene: se hai citato definizione + esempio + dato reale, sei pronto per l\'orale.',
    explainNo: 'Obiettivo: parla in modo ordinato. Se ti blocchi, riparti dalle parole chiave.'
  };
}

function makeRealContextQuestion(blockId, skillId){
  // Domanda "ponte" che forza un collegamento a un dato reale (curiosita'/confronto/piramide)
  const sid = skillId || '';
  if(sid.startsWith('G1_') && (sid.indexOf('PIR')!==-1 || sid.indexOf('piram')!==-1 || sid.indexOf('C4')!==-1 || sid.indexOf('C5')!==-1)){
    return {
      id: `ctx-${blockId}-${skillId}-qatar`,
      block: blockId,
      type: 'mcq',
      skill: skillId,
      prompt: 'Caso reale: Qatar (circa 2022) ha una popolazione con una quota altissima di 15-64 anni e pochi anziani. Quale spiegazione è la più coerente con il programma?',
      choices: [
        'Solo natalità altissima',
        'Forte immigrazione di lavoratori in età attiva (saldo migratorio positivo)',
        'Invecchiamento molto avanzato',
        'Assenza di migrazioni'
      ],
      answerIndex: 1,
      explainOk: 'Giusto: molti Paesi del Golfo hanno un forte afflusso di lavoratori in età attiva, che altera la "forma" della piramide.',
      explainNo: 'Ricollega: migrazioni + struttura per età. Se arriva tanta popolazione 15-64, la piramide si gonfia lì.',
      slideRef: 'DEMOGRAFIA_PIRAMIDE'
    };
  }
  if(sid.startsWith('G2_')){
    return {
      id: `ctx-${blockId}-${skillId}-siria`,
      block: blockId,
      type: 'mcq',
      skill: skillId,
      prompt: 'Caso reale: milioni di siriani si sono spostati in Turchia durante la guerra. Quale coppia descrive meglio push/pull factors?',
      choices: [
        'Push: opportunità di lavoro | Pull: guerra',
        'Push: guerra e insicurezza | Pull: protezione e servizi nel Paese di arrivo',
        'Push: clima mite | Pull: carestia',
        'Push: turismo | Pull: studio'
      ],
      answerIndex: 1,
      explainOk: 'Ottimo: push = ciò che ti spinge a partire; pull = ciò che ti attira verso la meta.',
      explainNo: 'Ricorda: push = espulsione (guerra, povertà), pull = attrazione (sicurezza, lavoro).',
      slideRef: 'MIGRAZIONI_PUSH_PULL'
    };
  }
  return null;
}

function buildChainForSkill(blockId, skillId){
  const bank = bankFor(blockId);
  if(!bank || !bank.length || !skillId) return [];

  // 0) se il prof/AI ha definito una catena esplicita nel template, usala
  try{
    const chains = loadStandardChains();
    const ch = (chains||[]).find(c=>c && c.skill===skillId && Array.isArray(c.steps) && c.steps.length);
    if(ch){
      const out=[];
      for(const st of ch.steps){
        if(!st || !st.type || !st.prompt) continue;
        if(st.type==='oral') out.push(makeOralQuestion(blockId, skillId, st.title||SKILL_TITLES[skillId]));
        else if(st.type==='mcq'){
          out.push({
            id: `chain-${blockId}-${skillId}-${Math.floor(Math.random()*1e9)}`,
            block: blockId,
            type:'mcq',
            skill: skillId,
            prompt: String(st.prompt),
            choices: Array.isArray(st.choices)?st.choices.slice(0,6):['A','B'],
            answerIndex: isFinite(Number(st.answerIndex))?Number(st.answerIndex):0,
            rule: st.rule||'',
            example: st.example||'',
            explainOk: st.explainOk||'',
            explainNo: st.explainNo||'',
            slideRef: st.slideRef||st.slideId||null
          });
        } else {
          out.push({
            id: `chain-${blockId}-${skillId}-${Math.floor(Math.random()*1e9)}`,
  ...
  // 1) definizione (mcq)
  const mcq = bank.filter(q=>q.type==='mcq' && skillOf(q)===skillId);
  const open = bank.filter(q=>(q.type==='open') && skillOf(q)===skillId);
  const step1 = mcq.length ? mcq[Math.floor(Math.random()*mcq.length)] : null;
  const step2 = open.length ? open[Math.floor(Math.random()*open.length)] : null;
  const step3 = makeRealContextQuestion(blockId, skillId);
  const step4 = makeOralQuestion(blockId, skillId, SKILL_TITLES && SKILL_TITLES[skillId] ? SKILL_TITLES[skillId] : 'Spiega il concetto');
  const out = [];
  if(step1) out.push(step1);
  if(step2) out.push(step2);
  if(step3) out.push(step3);
  if(step4) out.push(step4);
  return out;
}
function buildQueue(blockId, modeWanted){
  ...
  if(SMART){
    // -1) domande "iniettate" da strumenti (es. Piramide delle eta')
    if(Array.isArray(profile.injected) && profile.injected.length){
      const take = Math.min(2, profile.injected.length, N);
      for(let i=0;i<take;i++){
        const qx = profile.injected.shift();
        if(qx) pushIfNew(qx);
      }
      saveProfile(profile);
    }
  ...
  // 1.5) una "catena" sul concetto piu' debole (definizione -> applicazione -> realta' -> orale)
  // Serve a imparare in modo piu' profondo, non solo a cliccare.
  if(SMART){
    const CHAIN_ON = (set.chainMode !== false);
    if(CHAIN_ON && out.length < N){
      const b = blockById(blockId);
      const skillId = (b && b.skills && b.skills.length) ? weakestSkill(profile, b.skills) : null;
      const chain = buildChainForSkill(blockId, skillId);
      // inseriscila solo se c'e' spazio e non e' vuota
      if(chain && chain.length){
        for(const qx of chain){
...
function setRule(q){
  if(mode === "verify"){
    ...
    // in Verifica nascondiamo anche le curiosita'
    const cn = document.getElementById('curiosityNote');
...
/* -------------------- Collegamento slide/appunti --------------------
   Obiettivo: far vedere al prof che ogni domanda e' ancorata al SUO materiale.
   - Se la domanda contiene q.slideRef / q.slideTitle le usiamo.
   - Altrimenti inferiamo dal blocco/skill.
-------------------------------------------------------------------- */

const SLIDE_INDEX = {
  G1: {
    doc: "1) Demografia + Dinamiche demografiche (appunti)",
    bySkill: {
      "G1_C1": "Densita, distribuzione e fattori (geografici/umani/storici)",
      "G1_C2": "Saldi: naturale, migratorio, demografico",
      "G1_C3": "Tassi: natalita, mortalita, fecondita, mortalita infantile, speranza di vita",
      "G1_C4": "Piramide delle eta: lettura e interpretazione",
      "G1_C5": "Indicatori e indici (concetti base)"
    }
  },
  G2: {
    doc: "2) Le migrazioni (slide)",
    bySkill: {
      "G2_C1": "Migrante, migrazioni interne/internazionali, push/pull, regolari/irregolari",
      "G2_C2": "Italia: migrazioni interne e trasformazione in Paese d'immigrazione",
      "G2_C3": "Migrazioni contemporanee e globalizzazione (2000-2019)"
    }
  },
  G3: {
    doc: "3) Le citta (slide)",
    bySkill: {
      "G3_C1": "Urbanesimo e tasso di urbanizzazione",
      "G3_C2": "Definizione/funzioni/gerarchia urbana",
      "G3_C3": "Metropoli, area metropolitana, conurbazione, megalopoli",
      "G3_C4": "Modelli: nordamericane (CBD), europee, PVS (baraccopoli)"
    }
  }
};

function setSlideNote(q){
  const sn = $("slideNote");
  const st = $("slideText");
  if(!sn || !st) return;

  const blockId = (q && (q.block || q.unit || q.blockId)) ? (q.block || q.unit || q.blockId) : currentBlock;
  const skill = q && q.skill ? q.skill : null;

  // preferisci riferimenti espliciti (quando il prof/AI li inserisce)
  const explicit = (q && (q.slideRef || q.slideTitle)) ? true : false;
  if(explicit){
    // Se abbiamo il template standard importato, proviamo a risalire al documento del prof.
    try{
      const links = loadStandardSlideLinks();
      const sources = loadCustomSources();
      const ref = q.slideRef ? String(q.slideRef) : null;
      if(ref && links && links[ref]){
        const link = links[ref];
        const src = (sources||[]).find(s=>s && s.id===link.sourceId);
        const doc = src ? src.title : null;
        const title = q.slideTitle || link.title || ref;
        st.textContent = doc ? `${doc} • ${title}` : `${title} — ${ref}`;
        return;
      }
    }catch(e){}

    const parts = [];
    if(q.slideTitle) parts.push(q.slideTitle);
    if(q.slideRef) parts.push(q.slideRef);
    st.textContent = parts.join(" — ");
    return;
  }

  const idx = SLIDE_INDEX[blockId] || SLIDE_INDEX[currentBlock] || null;
  if(!idx){
    st.textContent = "—";
    return;
  }
  const hint = (skill && idx.bySkill && idx.bySkill[skill]) ? idx.bySkill[skill] : "Concetto del capitolo";
  st.textContent = `${idx.doc} • ${hint}`;
}

/* -------------------- Curiosita' (vita reale) --------------------
   Mostrate SOLO in Allenamento.
   - File: /data/curiosities.json (opzionale)
   - Fallback: piccola lista embedded
   Ogni curiosita' ha: text, sourceName, sourceUrl, blocks (opzionale)
------------------------------------------------------------------ */
...
const CURIOSITY_FALLBACK = [
  {
    id: "C_ITALY_DENSITY",
    blocks: ["G1"],
    text: "Lo sapevi che la densita media dell'Italia e di circa 195 abitanti per km^2? (valore indicativo)",
    sourceName: "World Bank (Population density - Italy)",
    sourceUrl: "https://data.worldbank.org/indicator/EN.POP.DNST?locations=IT"
  },
  {
    id: "C_REPLACEMENT_RATE",
    blocks: ["G1"],
    text: "Curiosita: la soglia di sostituzione e circa 2.1 figli per donna. Se un Paese resta sotto questa soglia per anni, tende a invecchiare (a parita di migrazioni).",
    sourceName: "OECD Data (fertility rates) - concetto 2.1",
    sourceUrl: "https://data.oecd.org/pop/fertility-rates.htm"
  },
  {
    id: "C_SYRIA_TURKEY",
    blocks: ["G2"],
    text: "Lo sapevi che la Turchia ospita milioni di rifugiati siriani registrati?",
    sourceName: "UNHCR - Refugees and Asylum Seekers in Turkiye",
    sourceUrl: "https://www.unhcr.org/tr/en/kime-yardim-ediyoruz/refugees-and-asylum-seekers-tuerkiye"
  },
  {
    id: "C_TOKYO_DENSITY",
    blocks: ["G3"],
    text: "Lo sapevi che Tokyo (prefettura) ha una densita di oltre 6.000 persone per km^2?",
    sourceName: "Nippon.com (basato su Statistics Bureau, MIC)",
    sourceUrl: "https://www.nippon.com/en/japan-data/h00782/"
  }
];



let CURIOSITIES = null; // lazy-loaded
let COMPARES = null;    // lazy-loaded
let _curiositySeen = [];
let _compareSeen = [];

async function loadCuriosities(){
  if(Array.isArray(CURIOSITIES) && Array.isArray(COMPARES)) return {curiosities:CURIOSITIES, compares:COMPARES};
  try{
    const res = await fetch('./data/curiosities.json', {cache:'no-cache'});
    if(!res.ok) throw new Error('HTTP '+res.status);
    const payload = await res.json();
    const arr = (payload && Array.isArray(payload.items)) ? payload.items : (Array.isArray(payload) ? payload : []);
    const all = arr.length ? arr : CURIOSITY_FALLBACK;
    const customItems = loadCustomCuriosities();
    const mergedAll = (customItems && customItems.length) ? (all.concat(customItems)) : all;
    // split: default kind = 'curiosity'
    CURIOSITIES = mergedAll.filter(it => !it || !it.kind || it.kind === 'curiosity');
    COMPARES = mergedAll.filter(it => it && it.kind === 'compare');
  }catch(e){
    CURIOSITIES = CURIOSITY_FALLBACK;
    COMPARES = [];
  }
  return {curiosities:CURIOSITIES, compares:COMPARES};
}

function pickCuriosityForBlock(blockId, items){
  const pool = items.filter(it => !it || !it.blocks || it.blocks.indexOf(blockId) !== -1);
  if(!pool.length) return null;
  // evita ripetizioni immediate
  const unseen = pool.filter(it => it && it.id && _curiositySeen.indexOf(it.id) === -1);
  const pickFrom = unseen.length ? unseen : pool;
  const it = pickFrom[Math.floor(Math.random()*pickFrom.length)];
  if(it && it.id){
    _curiositySeen.push(it.id);
    if(_curiositySeen.length > 30) _curiositySeen = _curiositySeen.slice(-30);
  }
  return it || null;
}

async function setCuriosity(blockId){
  const cn = document.getElementById('curiosityNote');
  if(!cn) return;
  if(mode !== 'train'){
  ...
  const loaded = await loadCuriosities();
  const it = pickCuriosityForBlock(blockId, loaded.curiosities || []);
  if(!it){
    cn.classList.add('hidden');
    return;
  }
  cn.classList.remove('hidden');
  const ct = document.getElementById('curiosityText');
  if(ct) ct.textContent = it.text || '—';
  const cs = document.getElementById('curiositySource');
  if(cs){
    const url = it.sourceUrl || (it.source && it.source.url) || '';
    const name = it.sourceName || (it.source && (it.source.ref || it.source.name)) || '';
    if(url){
      cs.innerHTML = `Fonte: <a href="${url}" target="_blank" rel="noopener noreferrer">${escapeHtml(name || 'link')}</a>`;
    } else {
      cs.textContent = name ? ('Fonte: '+name) : '';
    }
  }
}

// Confronto reale: mostrato DOPO la risposta (solo Allenamento)
function pickCompareForQuestion(blockId, q, items){
  const pool = items.filter(it => {
    if(!it) return false;
    if(it.blocks && it.blocks.indexOf(blockId) === -1) return false;
    if(it.skills && q && q.skill && it.skills.indexOf(q.skill) === -1) return false;
    return true;
  });
  if(!pool.length) return null;
  const unseen = pool.filter(it => it.id && _compareSeen.indexOf(it.id) === -1);
  const pickFrom = unseen.length ? unseen : pool;
  const it = pickFrom[Math.floor(Math.random()*pickFrom.length)];
  if(it && it.id){
    _compareSeen.push(it.id);
    if(_compareSeen.length > 30) _compareSeen = _compareSeen.slice(-30);
  }
  return it || null;
}

async function setCompare(blockId, q){
  const cn = $('compareNote');
  if(!cn) return;
  if(mode !== 'train'){
    cn.classList.add('hidden');
    return;
  ...
  const loaded = await loadCuriosities();
  const items = loaded.compares || [];
  const it = pickCompareForQuestion(blockId, q, items);
  if(!it){
    cn.classList.add('hidden');
    return;
  }
...
/* -------------------- Tool: Piramide delle eta' --------------------
   Mini-lab per disegnare un grafico a barre (maschi sinistra / femmine destra)
   Utile per imparare a leggere il grafico richiesto dal prof.
-------------------------------------------------------------------- */

let PYR_ROWS = [];
let PYR_PRESETS = null;
let PYR_PRESET_ACTIVE = null;

async function loadPyrPresets(){
  if(PYR_PRESETS) return PYR_PRESETS;
  try{
    const res = await fetch('data/pyramid_presets.json', {cache:'no-store'});
    const obj = await res.json();
    PYR_PRESETS = Array.isArray(obj) ? obj : (obj && obj.items ? obj.items : []);
  }catch(e){
    PYR_PRESETS = [];
  }
  // merge preset importati via template standard
  try{
    const extra = loadStandardPyramidPresets();
    if(Array.isArray(extra) && extra.length){
      const byId = new Map();
      for(const p of PYR_PRESETS) if(p && p.id) byId.set(p.id, p);
      for(const p of extra) if(p && p.id && !byId.has(p.id)) byId.set(p.id, p);
      PYR_PRESETS = Array.from(byId.values());
    }
  }catch(e){}
  return PYR_PRESETS;
}

function populatePyrPresetSelect(){
  const sel = $('pyrPreset');
  if(!sel) return;
  // evita duplicati
  const keepFirst = sel.querySelectorAll('option').length ? sel.querySelectorAll('option')[0] : null;
  sel.innerHTML = '';
  if(keepFirst){ sel.appendChild(keepFirst); }
  const base = document.createElement('option');
  base.value='';
  base.textContent='— Seleziona un caso (opzionale) —';
  sel.appendChild(base);
  (PYR_PRESETS||[]).forEach(p=>{
...
function applyPyrPreset(presetId){
  const p = (PYR_PRESETS||[]).find(x=>x.id===presetId) || null;
  PYR_PRESET_ACTIVE = p;
  if(!p){
    const box=$('pyrReality'); if(box){ box.classList.add('hidden'); box.innerHTML=''; }
    return;
  }
  if(Array.isArray(p.rows) && p.rows.length){
    PYR_ROWS = p.rows.map(r=>({age:r.age, m:Number(r.m)||0, f:Number(r.f)||0}));
    renderPyrTable();
  }
  const t=$('pyrTitle'); if(t && !t.value){ t.value = p.title || ''; }
  const box=$('pyrReality');
  if(box){
    const src = p.source && p.source.url ? `<a href="${escapeHtml(p.source.url)}" target="_blank" rel="noopener">${escapeHtml(p.source.ref||'Fonte')}</a>` : escapeHtml((p.source && p.source.ref) || '');
    box.innerHTML = `<b>Collegamento alla realtà</b><br>${escapeHtml(p.note||'')}${src?`<div class="muted tiny" style="margin-top:6px">Fonte: ${src}</div>`:''}`;
    box.classList.remove('hidden');
  }
  pyrRender();
}

function pyrDefaults(){
  return [
    {age:"0-14", m:13, f:12},
    {age:"15-64", m:34, f:36},
    {age:"65+", m:14, f:15}
...
function ensurePyrRows(){
  if(!Array.isArray(PYR_ROWS) || !PYR_ROWS.length) PYR_ROWS = pyrDefaults();
}

function populatePyrPresetSelect(){
  const sel = $('pyrPreset');
  if(!sel) return;
  // lascia la prima option
  const keep = sel.querySelectorAll('option').length ? sel.querySelectorAll('option')[0] : null;
  sel.innerHTML = '';
  if(keep) sel.appendChild(keep);
  (PYR_PRESETS||[]).forEach(p=>{
    const opt = document.createElement('option');
    opt.value = p.id || '';
    opt.textContent = p.title || p.name || p.id;
    sel.appendChild(opt);
  });
  // seleziona ultimo preset usato
  try{ sel.value = PYR_PRESET_ACTIVE || ''; }catch(e){}
}

function applyPyrPreset(presetId){
  const p = (PYR_PRESETS||[]).find(x=>x.id===presetId);
  if(!p) return;
  PYR_PRESET_ACTIVE = p.id;
  // carica righe
  if(Array.isArray(p.rows) && p.rows.length){
    PYR_ROWS = p.rows.map(r=>({age:r.age, m:Number(r.m)||0, f:Number(r.f)||0}));
  }
  // titolo
  const t=$('pyrTitle'); if(t) t.value = p.title || '';
  // box realta'
  const box=$('pyrReality');
  if(box){
    const note = p.note || '';
    ...
    let html = '';
    if(note) html += `<b>Collegamento alla realta'</b><br>${escapeHtml(note)}`;
    if(src && (src.url || src.name || src.ref)){
      const name = src.name || src.ref || 'Fonte';
      if(src.url) html += `<br><span class="muted tiny">Fonte: <a href="${src.url}" target="_blank" rel="noopener noreferrer">${escapeHtml(name)}</a></span>`;
      else html += `<br><span class="muted tiny">Fonte: ${escapeHtml(name)}</span>`;
    }
    box.innerHTML = html || '';
...
function renderPyrTable(){
  ...
  h.className = 'pyrRow';
  h.innerHTML = `<div class="muted tiny"><b>Eta'</b></div><div class="muted tiny"><b>Maschi</b></div><div class="muted tiny"><b>Femmine</b></div><div></div>`;
  wrap.appendChild(h);

  PYR_ROWS.forEach((r, idx)=>{
    const row = document.createElement('div');
    row.className = 'pyrRow';
    row.innerHTML = `
      <input data-pyr="age" data-idx="${idx}" type="text" value="${escapeHtml(r.age)}" />
      <input data-pyr="m" data-idx="${idx}" type="number" step="0.1" value="${r.m}" />
      <input data-pyr="f" data-idx="${idx}" type="number" step="0.1" value="${r.f}" />
      <button class="pyrDel" type="button" data-action="pyr-del" data-idx="${idx}">🗑</button>
    `;
    wrap.appendChild(row);
  });
}

function readPyrTable(){
  const wrap = $("pyrTable");
  if(!wrap) return;
  const inputs = wrap.querySelectorAll('input[data-pyr]');
  inputs.forEach(inp=>{
    const idx = Number(inp.getAttribute('data-idx'));
    const field = inp.getAttribute('data-pyr');
    if(!PYR_ROWS[idx]) return;
    if(field==='age'){
      PYR_ROWS[idx].age = String(inp.value||'').trim() || `Classe ${idx+1}`;
    } else {
      const v = Number(String(inp.value||'').replace(',','.'));
      PYR_ROWS[idx][field] = isFinite(v) ? v : 0;
    }
  });
}

function normalizePyr(modeSel){
  // returns {rowsPct, summaryText}
  const mode = modeSel || 'pct';
  const rows = PYR_ROWS.map(r=>({age:r.age, m:Number(r.m)||0, f:Number(r.f)||0}));
  let rowsPct = rows;
  if(mode==='abs'){
    const totM = rows.reduce((s,r)=>s+r.m,0) || 1;
    const totF = rows.reduce((s,r)=>s+r.f,0) || 1;
    rowsPct = rows.map(r=>({age:r.age, m:(r.m/totM)*100, f:(r.f/totF)*100}));
  }
  // clamp
  rowsPct = rowsPct.map(r=>({age:r.age, m:Math.max(0, r.m), f:Math.max(0, r.f)}));
  const sumM = rowsPct.reduce((s,r)=>s+r.m,0);
  const sumF = rowsPct.reduce((s,r)=>s+r.f,0);
  const lines = [];
  lines.push(`Totale maschi: ${sumM.toFixed(1)}% | Totale femmine: ${sumF.toFixed(1)}%`);
  lines.push(`Suggerimento lettura: base larga = popolazione giovane/alta natalita; cima larga = invecchiamento.`);
  return {rowsPct, summaryText: lines.join('\n')};
}

function drawPyrCanvas(title, rowsPct){
  const c = $("pyrCanvas");
  if(!c) return;
  ...
  const n = rowsPct.length || 1;
  const rowH = Math.max(18, Math.floor(chartH / n));

  const maxVal = Math.max(1, ...rowsPct.map(r=>Math.max(r.m, r.f)));
  const maxBar = Math.floor((w - pad*2)/2) - 20;

  // title
  ctx.font = '16px system-ui, -apple-system, Segoe UI, Roboto, Arial';
  ctx.fillStyle = '#111';
  try{ if(matchMedia('(prefers-color-scheme: dark)').matches) ctx.fillStyle = '#e5e7eb'; }catch{}
  ctx.fillText(title || 'Piramide delle età', pad, 24);

  // center line
  ctx.strokeStyle = 'rgba(148,163,184,.6)';
  ctx.beginPath();
  ctx.moveTo(mid, chartTop);
  ctx.lineTo(mid, chartBottom);
  ctx.stroke();
  ...
  // bars
  rowsPct.forEach((r,i)=>{
    const y = chartTop + i*rowH + 6;
    const mW = Math.round((r.m/maxVal)*maxBar);
    const fW = Math.round((r.f/maxVal)*maxBar);
    // male left
    ctx.fillStyle = 'rgba(37,99,235,.55)';
    ctx.fillRect(mid - mW, y, mW, rowH-10);
    // female right
    ctx.fillStyle = 'rgba(236,72,153,.45)';
    ctx.fillRect(mid, y, fW, rowH-10);

    // labels
    ctx.fillStyle = 'rgba(100,116,139,.95)';
    try{ if(matchMedia('(prefers-color-scheme: dark)').matches) ctx.fillStyle = 'rgba(203,213,225,.95)'; }catch{}
    ctx.fillText(r.age, pad, y+rowH-14);
    ctx.fillText(`${r.m.toFixed(1)}%`, mid - mW - 42, y+rowH-14);
    ctx.fillText(`${r.f.toFixed(1)}%`, mid + fW + 8, y+rowH-14);
  });

  // axis hint
  ctx.fillStyle = 'rgba(100,116,139,.9)';
  try{ if(matchMedia('(prefers-color-scheme: dark)').matches) ctx.fillStyle = 'rgba(203,213,225,.9)'; }catch{}
  ctx.fillText('Maschi', mid-60, chartBottom+26);
  ctx.fillText('Femmine', mid+16, chartBottom+26);
}

function openPyramidTool(){
  ensurePyrRows();
  const t = $("pyrTitle"); if(t) t.value = t.value || '';
  renderPyrTable();
  const sum = $("pyrSummary"); if(sum) sum.textContent = '';
  // carica preset reali
  loadPyrPresets().then(()=>{
    populatePyrPresetSelect();
    const sel=$('pyrPreset');
    if(sel && !sel._bound){
      sel._bound=true;
      sel.addEventListener('change', ()=>{
        const id=sel.value||'';
        if(!id){
          PYR_PRESET_ACTIVE=null;
          const box=$('pyrReality'); if(box){ box.classList.add('hidden'); box.innerHTML=''; }
          return;
        }
        // reset titolo solo se vuoto
        const tt=$('pyrTitle'); if(tt) tt.value='';
        applyPyrPreset(id);
      });
    }
  }).catch(()=>{});
  openModal('pyramidModal');
  // draw once
  setTimeout(()=>{ pyrRender(); }, 0);
}

function pyrRender(){
  readPyrTable();
  const modeSel = $("pyrMode");
  const mode = modeSel ? modeSel.value : 'pct';
  const t = $("pyrTitle");
  const title = t ? t.value : '';
  const norm = normalizePyr(mode);
  drawPyrCanvas(title, norm.rowsPct);
  const sum = $("pyrSummary");
  if(sum) sum.textContent = norm.summaryText;
}
...
function renderQuestion(){
  ...
  setRule(q);
  setSlideNote(q);
  // reset confronto reale: lo mostriamo solo dopo la risposta
  const cmp=$('compareNote'); if(cmp) cmp.classList.add('hidden');
  // Curiosita' contestuale (non blocca la UI)
  setCuriosity(currentBlock).catch(()=>{});
  setMeta(q);
...
function chooseMCQ(i, btn){
  ...
  // Dopo la risposta, in Allenamento mostriamo anche un confronto reale (se disponibile)
  setCompare(currentBlock, q).catch(()=>{});
...
function submitOpen(){
  ...
  const q = queue[qIndex];
  if(q && q.type === 'oral'){
    // Modalita' orale: non "correggiamo" in automatico, ma facciamo autovalutazione.
    $("feedback").textContent = explainRight(q) || 'Autovalutazione: hai dato definizione + esempio + dato reale + fonte?';
    // mostra comunque confronto reale se c'e'
    setCompare(currentBlock, q).catch(()=>{});
    // aggiorna SRS come "fatto" (ok) ma senza punti
    updateAfterAnswer(q.skill, true, q);
    checkUnlocks();
    $("nextBtn").disabled = false;
    return;
  }
...
function chooseGuidedMCQ(i, btn){
  ...
  setCompare(currentBlock, step).catch(()=>{});
...
document.querySelectorAll('[data-close="pyramid"]').forEach(el=>el.addEventListener('click',()=>closeModal('pyramidModal')));
...
document.addEventListener('click', (e) => {
  ...
  try{
    switch(action){
      case 'continue':
        // Evita loop: se il click è già sul bottone Continua, lascia fare al suo handler.
        if(btn && btn.id === 'saveNameBtn') return;
        // Altrimenti inoltra al bottone (una sola volta)
      ...
      case 'install-pwa':
        doInstallPwa();
        break;
      case 'reset-site-cache':
        resetSiteCacheAndReload();
        break;
      case 'open-pyramid':
        closeModal('settingsModal');
        openPyramidTool();
      ...
      case 'pyr-add':
        ensurePyrRows();
        PYR_ROWS.push({age:`${PYR_ROWS.length?PYR_ROWS[PYR_ROWS.length-1].age:''}` || `Classe ${PYR_ROWS.length+1}`, m:0, f:0});
        renderPyrTable();
        break;
      case 'pyr-del':
        {
          const idx = Number(btn.getAttribute('data-idx'));
          if(isFinite(idx) && PYR_ROWS[idx]){ PYR_ROWS.splice(idx,1); renderPyrTable(); }
        }
        break;
      case 'pyr-reset':
        PYR_ROWS = pyrDefaults();
        renderPyrTable();
        pyrRender();
        break;
      case 'pyr-render':
      ...
      case 'pyr-quiz':
        {
          // Genera 2 domande dal grafico/preset (salvate come injected per la prossima sessione)
          readPyrTable();
          const modeSel = $('pyrMode');
          const modeV = modeSel ? modeSel.value : 'pct';
          const norm = normalizePyr(modeV);
          const t = $('pyrTitle');
          const title = (t && t.value ? t.value.trim() : '') || (PYR_PRESET_ACTIVE && PYR_PRESET_ACTIVE.title) || 'Piramide delle età';
          ...
          const q1 = {
            ...
            type: 'oral',
            skill: 'G1_C4',
            prompt: `Commenta il grafico: ${title}\n\n1) Descrivi la forma (base/centro/cima)\n2) Cosa suggerisce su natalità e invecchiamento?\n3) Se noti squilibri, collega alle migrazioni.`,
            rule: 'Schema consigliato dal prof: descrivi → interpreta → collega a cause (natalità/mortalità/migrazioni).',
            explainOk: 'Autovalutazione: hai citato almeno 2 indizi del grafico + una causa?',
            slideRef: 'DEMOGRAFIA_PIRAMIDE'
          };
          ...
          const q2 = {
            ...
            type: 'mcq',
            skill: 'G1_C4',
            prompt: `In base a quanto vedi in ${title}, quale affermazione e' piu' plausibile?`,
            choices: [
              'Popolazione molto giovane (alta natalita)',
              'Popolazione anziana (invecchiamento forte)',
              "Popolazione sbilanciata da migrazioni in eta' attiva",
              "Non si puo' dire nulla dal grafico"
            ],
            answerIndex: (PYR_PRESET_ACTIVE && PYR_PRESET_ACTIVE.hint==='migration') ? 2 : ((PYR_PRESET_ACTIVE && PYR_PRESET_ACTIVE.hint==='aging') ? 1 : 0),
            explainOk: 'Giusto: il prof spesso chiede proprio di collegare la forma a natalita/invecchiamento/migrazioni.',
            explainNo: 'Riguarda base/centro/cima e pensa: giovane? anziana? o gonfia al centro per migrazioni?',
            slideRef: 'DEMOGRAFIA_PIRAMIDE'
          };

          if(!profile.injected) profile.injected = [];
          profile.injected.unshift(q2);
          profile.injected.unshift(q1);
          saveProfile(profile);
          toast('Ok: ho creato 2 domande. Le trovi come prime nella prossima sessione Allenamento.');
        }
        break;
      ...
      case 'validate-content':
        ...
        break;
            case 'download-content-template':
        downloadStandardTemplate();
        toast('Template scaricato.');
        break;
      case 'copy-content-template':
        copyStandardTemplate().catch(()=>toast('Impossibile copiare.'));
        break;
...
try{window.addEventListener("error",(e)=>{profile.lastError={message:e.message,source:e.filename,line:e.lineno,col:e.colno}; saveProfile(profile);});}catch{}
  ...
  if("serviceWorker" in navigator){
    // Add a versioned URL to force SW updates on GitHub Pages/CDN caches
    const swUrl = `./sw.js?v=${String(APP_VERSION||"v14").replace(/^v/,"")}`;
...
/* =========================================================
   Force-click delegation (fix "bottoni morti" su PWA/iOS)
   - Non dipende da bind per id; usa data-action in HTML
   ========================================================= */
(function forceActionDelegation(){
  function hasName(){
    try { return !!(localStorage.getItem(LS?.name || "quiz_user_name") || localStorage.getItem("quiz_user_name")); }
    catch(_) { return false; }
  }

  function ensureUnlocked(){
    // Se il nome esiste, forziamo home visibile e onboarding nascosto
    try{
      const name = localStorage.getItem(LS?.name || "quiz_user_name") || localStorage.getItem("quiz_user_name");
      if(name){
        if(typeof hide === "function") hide("onboarding");
        const onb = document.getElementById("onboarding"); if(onb) onb.style.display = "none";
        if(typeof show === "function") show("home");
        const home = document.getElementById("home"); if(home) home.style.display = "";
        if(typeof goHome === "function") { try{ goHome(); }catch(_){} }
      }
    }catch(_){}
  }
  ...
  document.addEventListener("click", (ev) => {
    ...
    // permette click anche su <a>
    ev.preventDefault();

    // Salvataggio nome: delega al bottone reale se esiste
    if(action === "save-name"){
      const real = document.getElementById("saveNameBtn");
      if(real) real.click();
      ensureUnlocked();
      return;
    }

    // Blocca azioni se non c'è nome (tranne install e open-settings)
    const allowWithoutName = ["install-pwa","open-settings"];
    if(!hasName() && !allowWithoutName.includes(action)){
    ...
    // Mappa azioni principali
    try{
      switch(action){
        case "start-train":
          if(typeof startRecommended === "function") return startRecommended();
          break;
        case "start-verify":
          if(typeof startVerify === "function") return startVerify();
          break;
        case "start-errors":
          if(typeof startErrors === "function") return startErrors();
          if(typeof showErrors === "function") return showErrors();
          break;
        case "go-progress":
          if(typeof showProgress === "function") return showProgress();
          break;
        case "open-settings":
          if(typeof openSettingsModal === "function") return openSettingsModal();
          if(typeof openSettings === "function") return openSettings();
          // fallback: prova ad aprire la sezione settings già presente
          const sm = document.getElementById("settings-modal");
          if(sm){ sm.classList.remove("hidden"); sm.setAttribute("aria-hidden","false"); return; }
          break;
        case "install-pwa":
          // se esiste un handler già definito, usalo
          if(typeof promptInstall === "function") return promptInstall();
          // fallback iOS
          alert("Su iPhone: Condividi → Aggiungi a Home");
          break;
      }
    }catch(e){
      alert("Errore azione: " + action + "\n" + (e?.message || e));
      console.error(e);
    }
  }, true);

  // All'avvio, se il nome c'è già, sblocca
  window.addEventListener("load", ensureUnlocked);
})();
...
/* =========================================================
   iOS PWA FIX v10: touch/click delegation + auto-save name
   Covers real ids/data-actions used by this app.
   ========================================================= */
(function iosPwaForceBindings_v10(){
  const isStandalone =
  ...
  function closestBtn(t){ return t && t.closest ? t.closest("button") : null; }

  function getNameInput(){
    return document.getElementById("nameInput") || document.getElementById("user-name");
  }

  function saveNameIfPossible(){
    const inp = getNameInput();
    const raw = (inp && inp.value ? inp.value : "").trim();
    if(!raw) return false;
    try{
      // Common keys used across builds
      const key = (typeof LS !== "undefined" && LS && LS.name) ? LS.name : "geo_name";
      localStorage.setItem(key, raw);
      // hide onboarding if present
      ...
      if(pill) pill.textContent = "Stato: pronto";
      // call app function if exists
      if(typeof setSafeName === "function") { try{ setSafeName(); }catch(_){ } }
      if(typeof goHome === "function") { try{ goHome(); }catch(_){ } }
      if(typeof goHomeFallback === "function") { try{ goHomeFallback(); }catch(_){ } }
      return true;
    }catch(e){
      return false;
    }
  ...
  function route(action, id){
    ...
    // Continue/name
    if(id === "saveNameBtn" || a === "continue"){
      saveNameIfPossible();
      return true;
    }

    // Start train / verify (by id or action)
    if(id === "trainBtn" || a === "start-train"){
      if(!saveNameIfPossible()){ /* ok even if already saved */ }
      if(typeof startRecommended === "function") { startRecommended(); return true; }
      if(typeof startTraining === "function") { startTraining(); return true; }
    }

    if(id === "verifyBtn" || a === "start-verify"){
      if(!saveNameIfPossible()){}
      if(typeof startVerify === "function") { startVerify(); return true; }
    }

    // Progress
    if(id === "progressBtn" || id === "navProgress" || a === "go-progress"){
      if(typeof goProgress === "function") { goProgress(); return true; }
      if(typeof goProgressFallback === "function") { goProgressFallback(); return true; }
      if(typeof showProgress === "function") { showProgress(); return true; }
    }

    // Settings
    if(id === "navSettings" || id === "settingsTopBtn" || a === "open-settings"){
      if(typeof openSettingsModal === "function") { openSettingsModal(); return true; }
      if(typeof openSettings === "function") { openSettings(); return true; }
      return true;
    }

    // Home
    if(id === "navHome" || a === "go-home"){
      if(typeof goHome === "function") { goHome(); return true; }
      if(typeof goHomeFallback === "function") { goHomeFallback(); return true; }
    }
  ...
  function handle(btn){
    ...
    try{
      ...
      alert("Errore PWA: " + (e && e.message ? e.message : e));
      console.error(e);
    }
  }

  document.addEventListener("click", (e)=>handle(closestBtn(e.target)), true);
  document.addEventListener("touchend", (e)=>handle(closestBtn(e.target)), {capture:true, passive:true});

  // If name already present but pill shows boot, fix it
  try{
    const key = (typeof LS !== "undefined" && LS && LS.name) ? LS.name : "geo_name";
    const stored = localStorage.getItem(key);
    if(stored){
      const pill = document.getElementById("statusPill");
      if(pill && /boot/i.test(pill.textContent || "")) pill.textContent = "Stato: pronto";
    }
  }catch(_){}
})();
...
/* =========================================================
   v14 - Robustezza: diagnostica, self-check, reset cache
   ========================================================= */

const DIAG = {
  lastErrorKey: "diag_last_error",
};
...
function setLastError(err) {
  try {
    ...
    localStorage.setItem(DIAG.lastErrorKey, JSON.stringify(payload));
  } catch (_) {}
}

window.addEventListener("error", (e) => {
  setLastError(e?.error || e?.message || "Script error");
});

window.addEventListener("unhandledrejection", (e) => {
  setLastError(e?.reason || "Unhandled rejection");
});

function showBanner(msg) {
  const b = document.getElementById("updateBanner");
  const t = document.getElementById("updateBannerText");
  const c = document.getElementById("updateBannerClose");
  if (!b || !t) return;
  t.textContent = msg;
...
async function resetCacheAndReload() {
  try {
    showBanner("Sto eliminando cache e aggiornando…");
    ...
    // Mantieni solo il nome utente se presente
    const keepName = localStorage.getItem("quiz_user_name");
    localStorage.clear();
    if (keepName) localStorage.setItem("quiz_user_name", keepName);
...
async function getDiagnosticsPayload() {
  const lastErrorRaw = localStorage.getItem(DIAG.lastErrorKey);
  let lastError = null;
  try { lastError = lastErrorRaw ? JSON.parse(lastErrorRaw) : null; } catch (_) {}
  ...
  const payload = {
    ...
    userNameStored: !!localStorage.getItem("quiz_user_name"),
    lastError,
    checks: {}
  };
  ...
  // quick fetch checks
  try {
    const r = await fetch("./data/G1.json", { cache: "no-store" });
    payload.checks.dataG1 = r.ok ? "OK" : ("HTTP " + r.status);
  } catch (_) {
    payload.checks.dataG1 = "FETCH_ERROR";
  }
...
async function runSelfCheck() {
  ...
  const ok = payload.checks.dataG1 === "OK";
  if (ok) {
    showBanner("✅ Controllo OK: dati caricabili e app pronta.");
    setTimeout(hideBanner, 2500);
  } else {
    showBanner("❌ Controllo fallito: " + payload.checks.dataG1 + ". Vai in Diagnostica → Elimina cache e aggiorna.");
  }
  return payload;
}
...
async function refreshDiagnosticsUI() {
...
function bindDiagnosticsUI() {
  ...
  if (closeBtn) closeBtn.onclick = closeDiagnostics;

  const selfBtn = document.getElementById("btn-selfcheck");
  if (selfBtn) selfBtn.onclick = async () => {
    await runSelfCheck();
    await refreshDiagnosticsUI();
  };
  ...
  if (copyBtn) copyBtn.onclick = async () => {
    ...
    showBanner("📋 Diagnostica copiata negli appunti.");
    setTimeout(hideBanner, 1500);
  };

  const resetBtn = document.getElementById("btn-reset-cache");
  if (resetBtn) resetBtn.onclick = resetCacheAndReload;

  const bannerClose = document.getElementById("updateBannerClose");
  if (bannerClose) bannerClose.onclick = hideBanner;

  // Apri diagnostica dall'icona impostazioni (fallback robusto)
  const gears = document.querySelectorAll('[data-action="open-settings"], [data-action="settings"], #navSettings, #settingsTopBtn, .settings-btn');
  gears.forEach((g) => {
    g.addEventListener("click", (e) => {
      // non bloccare se esiste gia' un pannello impostazioni interno; la diagnostica e' sempre utile
      openDiagnostics();
    }, true);
  });
}
...
window.addEventListener("load", () => {
  try { bindDiagnosticsUI(); } catch (_) {}
  try {
    const key = "diag_last_selfcheck_day";
